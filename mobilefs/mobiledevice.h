...
#define AMSVC_SYSTEM_PROFILER       CFSTR("com.apple.mobile.system_profiler")
  ...
  afc_error_t AFCConnectionOpen(int socket_fd, unsigned int io_timeout,
         ...
         */
        CFStringRef AMRecoveryModeCopyEnvironmentVariable(struct am_recovery_device *rdev, CFStringRef var);
  ...
  afc_error_t AFCFileRefSeek(struct afc_connection *conn, afc_file_ref ref,
                 unsigned long long offset, int origin);
  ...
  */
  ...
  unsigned int AMRestoreCreatePathsForBundle(CFStringRef restore_bundle_path,
        ...
        ramdisk_path);
  unsigned int AMRestoreModeDeviceReboot(struct am_restore_device *rdev); // Added by JB 30.07.2008
...
/*
  ...
  mach_error_t AMDPostNotification(SOCKET socket, CFStringRef  notification, CFStringRef userinfo);
  mach_error_t AMDObserveNotification(SOCKET socket, CFSTR notification);
  mach_error_t AMDListenForNotifications(SOCKET socket, NOTIFY_CALLBACK cb, USERDATA data);
  mach_error_t AMDShutdownNotificationProxy(SOCKET socket);
*/
